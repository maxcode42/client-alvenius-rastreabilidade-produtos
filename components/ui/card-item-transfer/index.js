...
export default function CardItemTransfer({ item, status, index, children }) {
  ...
  //           <CardItemsHeader
  //             index={index}
  //             acronym={`${item.status.length === status.length ? "FI" : "RE"}`}
  //             status={
  ...
  //                 : "Em trânsito"
  //             }
  //             text={statusList[item.status.length - 1]?.text
  //               .trimStart()
  //               .trimEnd()}
  //             bgSubStatus={statusList[item.status.length - 1]?.color}
  //           />
  //           <div className="flex flex-col w-full justify-center">
  //             <p className="w-full text-left px-2 py-1 text-xs sm:text-sm md:text-base">
  //               <strong>Código: </strong>
  ...
  function typeLayoutStatus(type) {
    item = {
      ...
      badge: type === "status_badge" ? true : false,
      status_acronym: item.status.length === status.length ? "FI" : "RE",
      status:
        item.status.length === status.length ? "Finalizado" : "Em trânsito",
      text: statusList[item.status.length - 1]?.text.trimStart().trimEnd(),
      bgSubStatus: statusList[item.status.length - 1]?.color,
    };
    ...
    const typesStatus = {
      status: () => {
        return (
          <CardItem index={index} item={item} className={"px-0 py-2"}>
            <div className="w-full flex flex-col py-1">
              <p className="w-full text-left px-2 text-xs sm:text-sm md:text-base">
                <small>
                  <strong>Fornecedor: </strong>
              ...
              <div className="flex flex-row w-full gap-1 items-center">
                {statusList.map((i, index) => (
                  ...
                  <div
                    ...
                    className={`relative
                        ${
                          item?.status_list?.includes(i.acronym)
                            ? i.color
                            : "bg-stone-200 text-stone-400"
                        }
                        text-[0.6em] md:text-[0.8em] text-left rounded-r-full
      ...
      status_badge: () => {
        return (
          <CardItem index={index} item={item} className={"px-2 py-2"}>
            <div className="w-full flex flex-col py-1">
              <p className="w-full text-left px-2 text-xs sm:text-sm md:text-base">
                <small>
                  <strong>Fornecedor: </strong>
